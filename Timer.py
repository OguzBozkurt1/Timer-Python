...
class Countdowntimer:
    def __init__(self):
        ...
        self.time_entry = tk.Entry(self.root, font=("Helvetica", 30),bg="Beige")
        self.time_entry.grid(row=0, column=0, columnspan=2, padx=5, pady=5)

        self.start_button = tk.Button(self.root, font=("Helvetica", 30), text="Start", command=self.start_thread,bg="Beige")
        self.start_button.grid(row=1, column=0, padx=5, pady=5)

        self.stop_button = tk.Button(self.root, font=("Helvetica", 30), text="Reset", command=self.stop,bg="Beige")
        self.stop_button.grid(row=1, column=1, padx=5, pady=5)

        self.time_label = tk.Label(self.root, font=("Helvetica", 30), text="Time: 00:00:00",bg="Beige")
    ...
    def start(self):
        ...
        if not self.stop_loop:
            toast = ToastNotifier()
            toast.show_toast("Countdown Timer", " Times UP !", duration=10)
